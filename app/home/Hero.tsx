    ...
    <div className="h-1.5 rounded-full bg-gray-100 overflow-hidden">
      <div
        ...
        style={{
          ...
          background: done
            ? "linear-gradient(90deg, #818cf8, #6366f1, #a78bfa, #6366f1, #818cf8)"
            : "#818cf8",
...
function KanbanBoard({ locale }: { locale: Locale }) {
  ...
  const prevProgressMap = useRef<Record<string, number>>(
    Object.fromEntries(
      INITIAL_COLUMNS.flatMap((c) => c.tasks.map((t) => [t.id, t.progress]))
    )
  );
    ...
    setColumns((prev) => {
      const newCols = prev.map((col) => {
        ...
        if (col.id === targetColId) {
          return {
            ...col,
            tasks: [...col.tasks, { ...task, progress: newProgress }]
          };
        }
        return col;
      });
            ...
            >
              ...
              {tasks.map((task, taskIdx) => (
                <div
                  ...
                  style={{
                    opacity:
                      draggedTask?.task.id === task.id ? 0.4 : mounted ? 1 : 0,
                    transform:
                  ...
                  <div className="flex items-center gap-1 mt-1.5">
                    ...
                    <div className="relative flex-1">
                      <ProgressBar
                        ...
                        from={
                          mounted ? (prevProgressMap.current[task.id] ?? 0) : 0
                        }
...
export function Hero({ locale = "en" }: { locale?: Locale }) {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(160deg,#f9d4b0 0%,#f5c5a0 15%,#f0d4c0 35%,#e8d0d8 55%,#d4c8e8 75%,#c8b8e0 100%)"
      }}
    >
