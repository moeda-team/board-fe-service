...
> = {
  en: {
    ...
    freeFeatures: ["Unlimited members", "Unlimited spaces", "All features included", "Priority support", "Regular updates"],
    ...
    basicFeatures: ["Up to 10 members", "Up to 5 workspaces", "5 GB storage", "Email support"],
    ...
    proFeatures: ["Advanced team management", "Advanced permissions & roles", "Activity logs & audit trail", "Custom integrations", "Advanced security", "Priority support"],
    ...
    customFeatures: ["Everything in Pro", "Custom security & compliance", "SSO & SAML", "Dedicated account manager", "Custom integrations", "SLA & enterprise support"],
  ...
  id: {
    ...
    freeFeatures: ["Member tanpa batas", "Space tanpa batas", "Semua fitur termasuk", "Dukungan prioritas", "Update berkala"],
    ...
    basicFeatures: ["Hingga 10 anggota", "Hingga 5 workspace", "5 GB penyimpanan", "Dukungan email"],
    ...
    proFeatures: ["Manajemen tim lanjutan", "Permission & role lanjutan", "Log aktivitas & jejak audit", "Integrasi kustom", "Keamanan lanjutan", "Dukungan prioritas"],
    ...
    customFeatures: ["Semua di Pro", "Keamanan & kepatuhan kustom", "SSO & SAML", "Account manager khusus", "Integrasi kustom", "SLA & dukungan enterprise"],
...
function PriceDisplay({
  ...
  if (!plan) {
    return <span className={`text-3xl font-bold leading-none ${dark ? "text-white" : "text-gray-900"}`}>{fallback}</span>;
  }

  const hasAnchoring = plan.originalPrice != null && plan.originalPrice > plan.basePrice;
  const hasPromo = plan.campaignName != null && isCampaignActive(plan.campaignEndDate);
  const durationLabel = plan.durationDays === 0 ? c.forever : `${plan.durationDays}d`;
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <div className="flex items-end gap-1.5">
        <span className={`text-3xl font-bold leading-none ${dark ? "text-white" : "text-gray-900"}`}>
          {plan.basePrice === 0 ? "Free" : formatIDR(plan.basePrice)}
        </span>
        {plan.basePrice > 0 && (
          <span className={`text-sm mb-0.5 ${dark ? "text-gray-400" : "text-gray-500"}`}>
            /{durationLabel}
          </span>
        )}
      </div>
    ...
    >
      <div ref={r.ref} className="max-w-6xl mx-auto w-full">
        ...
        <div className={`text-center ${standalone ? "mb-8" : "mb-12"} reveal-up ${r.visible ? "revealed" : ""}`}>
          ...
          <p className="text-gray-500 max-w-md mx-auto mt-3 text-[15px]">{c.subtitle}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 items-center">
          ...
          <div className={`bg-white rounded-2xl border border-gray-200 flex flex-col shadow-sm p-6 reveal-up stagger-1 ${r.visible ? "revealed" : ""}`}>
            ...
            <Link href="/login" className="w-full text-center py-3 rounded-xl text-sm font-semibold bg-blue-500 text-white hover:bg-blue-600 transition-colors">
              {c.freeCta}
            </Link>
          ...
          {/* ── Card 2: Basic ────────────────────────────────── */}
          <div className={`bg-white rounded-2xl border border-gray-200 flex flex-col shadow-sm p-6 reveal-up stagger-2 ${r.visible ? "revealed" : ""}`}>
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-1">{c.basicName}</h3>
            <PriceDisplay plan={basicPlan} fallback={c.basicDefaultPrice} locale={locale} />
            ...
            <Link href="/payment?plan=basic" className="w-full text-center py-3 rounded-xl text-sm font-semibold bg-teal-500 text-white hover:bg-teal-600 transition-colors">
              {c.basicCta}
            </Link>
            <p className="flex items-center justify-center gap-1 text-[11px] text-gray-400 mt-4">
          ...
          <div className={`rounded-2xl flex flex-col bg-[#111827] shadow-2xl overflow-hidden -my-5 z-10 reveal-up stagger-3 ${r.visible ? "revealed" : ""}`}>
            <div className="flex items-center justify-center gap-1.5 pt-4 pb-3 px-6">
              <span className="text-sm">⭐</span>
              <span className="text-[12px] font-semibold px-2.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.12)", color: "#e5e7eb" }}>
                {c.recommended}
              </span>
            </div>
            <div className="flex flex-col flex-1 px-6 pb-6">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">{c.proName}</span>
              <PriceDisplay plan={proPlan} fallback={c.proDefaultPrice} locale={locale} dark />
              ...
              <Link href="/payment?plan=pro" className="w-full text-center py-3.5 rounded-xl text-sm font-semibold bg-white text-gray-900 hover:bg-gray-100 transition-colors">
                {c.proCta}
              </Link>
          ...
          {/* ── Card 4: Custom ────────────────────────────────── */}
          <div className={`bg-white rounded-2xl border border-gray-200 flex flex-col shadow-sm p-6 reveal-up stagger-4 ${r.visible ? "revealed" : ""}`}>
            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-4">{c.customName}</span>
            <h3 className="text-4xl font-bold text-gray-900 mb-1">{c.customPrice}</h3>
            ...
            <div className="flex items-start gap-3 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-5">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 mb-0.5">Tailored for you</p>
                <p className="text-xs text-gray-400">Contact our sales team for a custom quote.</p>
              </div>
            </div>
            <ul className="space-y-3 flex-1 mb-6">
            ...
            <Link
              href="mailto:sales@papanclip.com?subject=Custom%20Plan%20Inquiry"
            ...
            <p className="text-center text-xs text-gray-400 mt-3">{c.customReply}</p>
            <p className="flex items-center justify-center gap-1 text-[11px] text-gray-400 mt-4">
