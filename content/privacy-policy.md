# Privacy Policy

Privacy Policy

Task Management Application

Last Updated: 13 April 2026

1. Introduction

Welcome to PapanClip. We are committed to protecting the privacy and security of your personal information. This Privacy Policy explains how we collect, use, disclose, and safeguard information when you use our Task Management Application.

By accessing or using the Application, you agree to the practices described in this Privacy Policy.

⸻

2. Information We Collect

2.1 Information You Provide

We may collect information that you voluntarily provide, including:

* Full name
* Email address
* Username and profile information
* Job title and department
* Organization or company information
* Profile photos or avatars
* Task descriptions, comments, attachments, and project-related data
* Communications submitted through support channels

2.2 Information Collected Automatically

When you use the Application, we may automatically collect:

* IP address
* Browser type and version
* Device information
* Operating system
* Login timestamps and activity logs
* Application usage statistics
* Cookies and similar tracking technologies

2.3 Third-Party Integrations

If enabled by your organization, we may receive information from third-party services integrated with the Application, such as:

* Single Sign-On (SSO) providers
* Calendar applications
* Communication platforms
* File storage providers

⸻

3. How We Use Your Information

We use collected information to:

* Provide and maintain the Application.
* Create and manage user accounts.
* Facilitate task assignment, tracking, and collaboration.
* Monitor project progress and team performance.
* Generate reports and analytics.
* Improve system functionality and user experience.
* Provide customer support.
* Detect, investigate, and prevent security incidents and unauthorized access.
* Comply with legal obligations.

⸻

4. Data Sharing and Disclosure

We do not sell your personal information.

We may share information under the following circumstances:

With Your Organization

If your account is provided through your employer or organization, administrators may access and manage information associated with your account, including:

* User profiles
* Assigned tasks
* Project activities
* Performance and productivity reports
* Audit logs

Service Providers

We may share information with trusted third-party vendors that assist us in operating the Application, including providers of:

* Cloud hosting services
* Analytics services
* Customer support tools
* Security monitoring services

These providers are contractually obligated to protect your information.

Legal Requirements

We may disclose information when required to:

* Comply with applicable laws and regulations;
* Respond to lawful requests from public authorities;
* Protect our rights, users, and systems;
* Investigate fraud or security incidents.

⸻

5. Data Retention

We retain personal and organizational data only as long as necessary to:

* Provide the services requested;
* Fulfill contractual obligations;
* Comply with legal and regulatory requirements;
* Resolve disputes and enforce agreements.

Organizations may request deletion of their data in accordance with applicable laws and contractual agreements.

⸻

6. Data Security

We implement reasonable administrative, technical, and organizational safeguards to protect your information, including:

* Encryption of data in transit using SSL/TLS;
* Role-based access control (RBAC);
* Multi-factor authentication (where enabled);
* Regular security monitoring and audits;
* Secure backup and disaster recovery procedures.

However, no method of electronic transmission or storage is completely secure, and we cannot guarantee absolute security.

⸻

7. User Rights

Depending on applicable laws, you may have the right to:

* Access personal information we hold about you;
* Correct inaccurate or incomplete information;
* Request deletion of your personal information;
* Restrict or object to certain processing activities;
* Request a copy of your data in a portable format;
* Withdraw consent where processing is based on consent.

Requests can be submitted through the contact information provided below.

⸻

8. Cookies and Tracking Technologies

The Application may use cookies and similar technologies to:

* Maintain user sessions;
* Remember preferences;
* Analyze usage patterns;
* Improve performance and functionality.

You may control cookie settings through your browser settings; however, disabling cookies may affect certain Application features.

⸻

9. International Data Transfers

If information is transferred or processed outside your country of residence, we will take appropriate measures to ensure that such transfers comply with applicable data protection laws and maintain adequate safeguards.

⸻

10. Children’s Privacy

The Application is intended for business and professional use and is not directed toward individuals under the age of 18. We do not knowingly collect personal information from children.

⸻

11. Changes to This Privacy Policy

We may update this Privacy Policy periodically. Any changes will be posted within the Application, and the “Last Updated” date will be revised accordingly.

Continued use of the Application after changes become effective constitutes acceptance of the updated Privacy Policy.

⸻

12. Contact Us

If you have questions, concerns, or requests regarding this Privacy Policy or our data practices, please contact us:

PapanClip
Email: papanclip.official@gmail.com
Address: Bandung, West Java, Indonesia
Phone: -

⸻

13. Compliance Statement

This Privacy Policy is designed to align with applicable data protection regulations, including but not limited to:

* General Data Protection Regulation (GDPR)
* California Consumer Privacy Act (CCPA), where applicable
* Applicable local data protection laws and regulations

⸻

By using the Task Management Application, you acknowledge that you have read and understood this Privacy Policy.